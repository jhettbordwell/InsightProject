...
def mapReview2SEclusters(condition, answer=None, cutoff=100):
    ...
    feature_frame = pd.concat([feature_frame, (dataframe['Effectiveness']-1)/4.], axis=1)
...
def get_clusters(condition='Depression', justKMeans=False):
    ...
    feature_frame = feature_frame.drop(columns='Effectiveness')
    feature_frame = feature_frame.drop(index=[ind for ind in feature_frame.index if feature_frame.loc[ind].sum() == 0])
...
def CosSim(v1, v2, weights=None):
    if weights == None: weights = np.ones(v1.size)
    num = np.sum(v1*v2*weights)
    den = np.sqrt(np.sum(v1**2 * weights) * np.sum(v2**2 * weights))
    return num/den

def binaryCompare(v1,v2,weights=None):
    if weights == None:
        weights = np.ones(v1.size)
    else:
        weights = np.array(weights)
    compare = v1 == v2
    score = (compare*weights).sum() / weights.sum() / v1.size
    return score
...
def rankbyTwo(one, two):
    ...
    for v in np.unique(vals):
        if (vals == v).sum() == 1:
            True_inds.append(inds[vals==v])
        else:
            many_inds = inds[vals==v]
            chunk = two[many_inds]
            new_inds = np.argsort(chunk)
            True_inds.append(new_inds)

    True_inds = np.hstack(True_inds)
    return True_inds
            

def rank_meds2(CS1, CS2, meds, clusters):
    # Using the compatibility score of each cluster, rank the medications based on which are best
    # represented in the most compatible cluster
    umeds = np.unique(meds)
    sorted_meds = []
    for med in umeds:
        ...
        clusters = clusters.flatten()
        med_clust = clusters[inds]
        fracs = [(med_clust==i).sum()/len(med_clust) for i in range(len(CS1))]
        sorted_meds.append(fracs)
    
    clust_ranked_inds = rankbyTwo(CS1, CS2)[::-1]
    meds_best_clust = np.array(sorted_meds).T[clust_ranked_inds][0]
    ranked_meds = umeds[meds_best_clust.argsort()[::-1]]
...
def RBO_score_methods(krange, featInds, user_weights, CS, medications, condition):
    ...
    for k in krange:
        ...
        for mtd in methods:
            ...
            # Creating a comparison vector that has no side effects and full effectiveness
            #comparison = np.zeros(cfw.shape[0])
            #comparison[-1] = 1
            
            # CS1 = np.array([CosSim(v, comparison[featInds], user_weights) for v in cfw[featInds].T])
            # CS2 = np.array([CosSim(v, comparison) for v in cfw.T])
            #CS1 = np.array([binaryCompare(v, comparison[featInds], user_weights) for v in cfw[featInds].T])
            #CS2 = np.array([binaryCompare(v, comparison) for v in cfw.T])
            
            #mtdList = rank_meds2(CS1, CS2, medications, clusters)
...
if __name__=="__main__":
    ...
    # Test on ADHD
    answer = fake_answer()
    CS, inds, ratings, meds = mapReview2SEclusters('ADHD', answer=answer)
    krange = np.arange(2,15)

    scores = RBO_score_methods(krange, inds, ratings, CS, meds, 'ADHD')
    make_RBO_vs_k_plot(scores, 'ADHD')

...
def parseSE_FAERs(file, meds):
    ...
    return clean_SEs, sideEff['Synonyms']
...
def find_sideEffects_match2clean(revFile, sefile1, sefile2, faers=True):
    ...
    listSEs1, Faers_syns = parseSE_FAERs(sefile1, meds)
    listSEs2, listfullSEorig = parseSEorig(sefile2)
    ...
    return listSEs1, listSEs2, Faers_syns, listfullSEorig
...
for condition in ['ADHD', 'Anxiety', 'Bipolar-Disorder', 'Depression', 'Schizophrenia']:
    ...
    if not glob.glob('SideEffectMatching/{:s}_processed.csv'.format(condition)):
        ...
        for i, med in enumerate(medications):
            ...
            print(condition, med)
            cleanFaers, cleanOrig, SynsFaers, listfullSEorig = find_sideEffects_match2clean('ProcessedReviews/{:s}/{:s}_{:s}_parsed_reviews.csv'.format(condition, med.strip(), condition), 
                                                                                            'NERstuff/faers_results/{:s}/SideEffectsExtracted.csv'.format(condition),
                                                                                            'SideEffects/{:s}_SideEffects.csv'.format(condition))

            clean_SEs = cleanFaers+cleanOrig
            Full_SEs = list(SynsFaers.values)+ list(listfullSEorig)
            master_df = pd.DataFrame([{'Cleaned': ', '.join(SE), 'Full': fSE} for SE, fSE in zip(clean_SEs, Full_SEs)])
        ...
        monster_df.to_csv('SideEffectMatching/{:s}_processed.csv'.format(condition), sep='$')

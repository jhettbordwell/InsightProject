"""
To work with the information in the FAERs reports, I utilized the FAERS package created by mlbernauer
on github (https://github.com/mlbernauer/FAERS), which made my life easier by providing a script to 
wget the quarterly reports (I just had to modify the input to grab everything), and especially by cleanly
processing the reports into a SQL database, which I then queried using sqlalchemy and pandas with psycopg2.

This code goes through every condition, and for every medication, pulls all the FAERs reports and writes a 
dataframe with the sql table info pivoted and aggregated in pandas appropriately.
"""
...
# For every unique medication, I pulled all the FAERs reports and the useful info, pivoted the tables
# and saved the results in a massive dataframe
...
# Lithium had to be treated special because in FAERs it's LITHIUM, and everywhere else it's lithium carbonate
